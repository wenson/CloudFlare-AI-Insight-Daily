...
**AI 资讯日报** 是一个基于 **Cloudflare Workers** 驱动的内容聚合与生成平台。它每日为您精选 AI 领域的最新动态，包括行业新闻、前沿学术论文、科技大V社交媒体言论，并通过 AI 模型进行智能处理与摘要生成，直接在 Worker 侧完成日报、播客稿和分析内容的生成。

我们的目标是成为您在瞬息万变的 AI 浪潮中保持领先的得力助手，让您高效获取最有价值的信息。

> [!NOTE]
> 日报后端项目已发布2.0: [PrismFlowAgent](https://github.com/justlovemaki/PrismFlowAgent) , 原生支持AI操作，可docker部署
> 
...
*   **🗓️ 可接入定时任务**：可通过 Cloudflare Cron、外部调度器或手动触发的方式稳定生成每日内容。
*   **🔧 高度可扩展**：项目架构灵活，不仅限于 AI 领域，您可以轻松定制，将其改造为您专属的任何主题日报。请尽情发挥您的想象力！
*   **📝 原生生成日报与播客稿**：抓取、勾选、AI 生成与页面预览都在同一套 Worker 流程内完成。
*   **📡 内置 RSS 输出**：日报生成成功后会自动写入 D1，并通过 Worker 直接输出 RSS。
...
### 🧑‍💻 AI 从业者与研究者
> **痛点：** 信息海洋无边无际，筛选关键动态、前沿论文和高价值观点耗时费力。
...
部署后可直接订阅你的 Worker RSS：

*   **订阅链接**
    > `https://<your-worker-domain>/rss`
    >
    > `⭐ 支持最近 N 天聚合，可用 /rss?days=7`
...
*   **🗺️ [数据源与数据流说明](docs/DATA_FLOW.md)**：了解数据从哪里来、如何写入 KV、如何进入 AI 生成主链路。
*   **🧭 [接口路由图](docs/API_ROUTES.md)**：查看 Worker 当前暴露的主要接口、职责分组与主调用路径。
*   **🗄️ [KV 键结构说明](docs/KV_KEYS.md)**：查看项目使用了哪些 KV key、何时写入、何时读取、TTL 多长。
...
4.  **技术风险**：本项目基于 Cloudflare Workers、Cloudflare KV 及第三方 AI / 内容服务运行。我们无法保证这些服务的永久稳定性和可用性。因任何技术故障、网络问题、服务中断或不可抗力导致的损失，本项目不承担任何责任。
